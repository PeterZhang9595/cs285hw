...
class SoftActorCritic(nn.Module):
    ...
    def get_action(self, observation: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            action: torch.Tensor = action_distribution.sample() # Here we don't need to update parameters, so 'sample' is ok.
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_action = next_action_distribution.rsample()  # Use rsample for reparameterization
            ...
            target_values = reward[None,:] + self.discount * (1 - done[None,:].float()) * next_qs
            # ENDTODO
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        q_values = self.critic(obs, action) # replace this with the correct q_values
        assert q_values.shape == (self.num_critic_networks, batch_size), q_values.shape
        # ENDTODO
        ...
        loss = -q_values.mean() # replace this with the correct loss
        # ENDTODO
